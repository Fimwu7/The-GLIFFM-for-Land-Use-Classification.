# Backbone: TransXnet
# Description:
# Our GLIFFM utilizes TransXNet as the backbone to extract comprehensive global-local features.
# The implementation of this backbone is adapted from the official TransNeXt repository.
# Source: https://github.com/LMMMEng/TransXNet
# Reference:
# M. Lou et al., "TransXNet: Learning Both Global and Local Dynamics With a Dual
# Dynamic Token Mixer for Visual Recognition," IEEE Transactions on Neural
# Networks and Learning Systems (TNNLS), vol. 36, no. 6, pp. 11534-11547, 2025.
...
class LCPatchEmbed(nn.Module):
    ...
    def __init__(self,
        ...
        # Land cover prediction model (can be independent of GLIFFM)
        self.landcover_model = smp.UnetPlusPlus(
            in_channels=3,
            classes=6,
            encoder_name='resnet34'
        )
        self._init_landcover_model(r'/savemodels/seg/UnetPlusPlus.pth')

        # Dual-branch projection
        self.rgb_proj = ConvModule(
            in_chans,
            embed_dim,
        ...
        # MFE
        self.f = MFE(embed_dim)

    def _init_landcover_model(self, ckpt_path):
        state_dict = torch.load(ckpt_path)
        self.landcover_model.load_state_dict(state_dict)
        for name, param in self.landcover_model.named_parameters():
            param.requires_grad_(False)

    def forward(self, rgb):
        lc = self.landcover_model(rgb)
        rgb_feat = self.rgb_proj(rgb)
        lc_feat = self.lc_proj(lc)
        out = self.f(rgb_feat, lc_feat)
        return out
...
class HybridTokenMixer(nn.Module):  ### D-Mixer
    def __init__(self,
        ...
        # LGFEM and GIFB
        self.feature_en = LGFEM(dim // 2)
        self.f = GIF(dim, fusion_groups=4)

    def forward(self, x, relative_pos_enc=None):
        x1, x2 = torch.chunk(x, chunks=2, dim=1)
        x1 = self.local_unit(x1)
        x2 = self.global_unit(x2, relative_pos_enc)
        x = self.feature_en(x1, x2)
        x = self.f(x)
        return x
...
class TransXNet(nn.Module):
    ...
    Args:
        ...
        out_indices (Sequence | int): Output from which MSCN position.
    ...
    def __init__(self,
        ...
        self.patch_embed = LCPatchEmbed(patch_size=in_patch_size,
                                        stride=in_stride,
                                        padding=in_pad,
                                        in_chans=in_chans,
...
@register_model
def transxnet_t(pretrained=True, pretrained_cfg=None, **kwargs):
